...
struct RecipeDetailView: View {
    @EnvironmentObject var viewModel: RecipeSearchViewModel
    @State private var selectedServings: Int = 0  // 用于选择份量
    let recipeId: Int
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    if let recipe = viewModel.selectedRecipe {
                        ...
                        // 调整份量
                        SectionView(title: "Select your serving size") {
                            VStack(alignment: .leading) {
                                
                                Picker("Select your serving size", selection: $selectedServings) {
                                    ForEach(servingOptions, id: \.self) { option in
                                        Text("\(option)").tag(option)
                                    }
                                }
                                .pickerStyle(SegmentedPickerStyle())
                                .onChange(of: selectedServings) { newValue in
                                    updateServings()
                                }
                            }
                            .padding(.horizontal)
    ...
    // 將更新份量的邏輯提取出來
    private func updateServings() {
                viewModel.adjustServings(newServings: selectedServings)
            }
}
